...
1. EydiiVerifyTool — explicit verification tool the agent can call:
...
2. eydii_before_tool_callback — intercepts ALL tool calls automatically:
...
3. eydii_wrap_tool — decorator wrapping individual tools:
...
logger = logging.getLogger("eydii_google_adk")


class EydiiVerifyTool:
    """Google ADK tool that verifies agent actions through EYDII policies.
    ...
    def as_tool(self) -> Callable:
        ...
        Usage::
            ...
            eydii = EydiiVerifyTool(policy="finance-controls")
            agent = Agent(name="bot", tools=[eydii.as_tool()])
        """
...
def eydii_before_tool_callback(
    api_key: Optional[str] = None,
    base_url: str = "https://veritera.ai",
    agent_id: str = "google-adk-agent",
    policy: Optional[str] = None,
    ...
    """Create a Google ADK before_tool_callback that verifies tool calls through EYDII.

    Intercepts every tool call before execution. If EYDII denies the action,
    ...
    def _before_tool(
        ...
        """
        if tool_name in eydii.skip_actions:
            logger.debug("EYDII: skipping verification for %s", tool_name)
            return None

        safe_args = tool_args if isinstance(tool_args, dict) else {"raw": str(tool_args)}

        try:
            result = eydii._client.verify_sync(
                action=tool_name,
                agent_id=eydii.agent_id,
                params=safe_args,
                policy=eydii.policy,
            )
        except Exception as exc:
            logger.error("EYDII verify error for %s: %s", tool_name, exc)
            if eydii.fail_closed:
                if eydii.on_blocked:
                    eydii.on_blocked(tool_name, str(exc))
                return {
        ...
        if result.verified:
            ...
            if eydii.on_verified:
                eydii.on_verified(tool_name, result)
            return None  # Allow execution

        reason = result.reason or "Policy violation"
        logger.warning("EYDII DENIED: %s — %s", tool_name, reason)
        if eydii.on_blocked:
            eydii.on_blocked(tool_name, reason)
        return {
            "error": f"Action '{tool_name}' denied by EYDII: {reason}",
            "eydii_status": "denied",
...
def eydii_after_tool_callback(
    api_key: Optional[str] = None,
    base_url: str = "https://veritera.ai",
    agent_id: str = "google-adk-agent",
    policy: Optional[str] = None,
) -> Callable:
    """Create a Google ADK after_tool_callback that audits tool results through EYDII.

    Logs every tool execution result to the EYDII audit trail.
    This callback is non-blocking — it never prevents execution.

    Usage::

        from eydii_google_adk import eydii_after_tool_callback

        agent = Agent(
            name="finance-bot",
            tools=[...],
            after_tool_callback=eydii_after_tool_callback(policy="audit-trail"),
        )
    ...
    def _after_tool(
        ...
        Audits the tool execution through EYDII. Non-blocking.
        ...
        try:
            ...
            logger.debug("EYDII after_tool audit error (non-blocking): %s", exc)
...
def eydii_wrap_tool(
    ...
    """Decorator that wraps a Google ADK tool function with EYDII verification.

    The original function is only called if EYDII approves the action.
    ...
    Returns:
        A decorator that wraps tool functions with EYDII verification.
    """
    eydii = EydiiVerifyTool(
        api_key=api_key,
        base_url=base_url,
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def async_wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            try:
                result = await eydii._client.verify_decision(
                    action=tool_name,
                    agent_id=eydii.agent_id,
                    params=params,
                    policy=eydii.policy,
